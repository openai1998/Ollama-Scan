...
def JSON_load(text):
    ...
    # 提取ip和port信息
    ip_port_list = [
        (match["portinfo"]["hostname"], match["portinfo"]["service"], match["ip"], match["portinfo"]["port"]) for match in data["matches"]
    ]
    if ip_port_list == []:
...
def Key_Test(key, proxies, choices, searchs):
    ...
    Headers = {"API-KEY": key, "Content-Type": "application/x-www-form-urlencoded"}
    keytesturl = 'https://api.zoomeye.org/host/search?query=app:"Docker remote API"&page=1'
    try:
        requests.packages.urllib3.disable_warnings()
...
def ZoomDowload(key, proxies):
    cprint("======开始对接ZoomEye接口进行Docker资产测绘======", "green")
    ...
    try:
        ...
        print("请不要输入无意义的字符串")
        sys.exit()
    search = input('[.] 请输入要测绘的语句（默认app:"Docker remote API"）: ')
    if search == "":
        searchs = str('app:"Docker remote API"')
    else:
        searchs = str(search)
